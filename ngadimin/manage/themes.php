...
// read isi readme 
$fileComment = "content/themes/$theme/$theme.txt"; 
...
$admin .='<div class="border">';
$admin .= '<img class="img-thumbnail pull-left" src="content/themes/'.$theme.'/'.$theme.'.jpg" width="180"><h4>'.$theme.'</h4>'.$readmetheme.'';
$admin .='</div>';
$admin .= '<h4 class="bg">Available Themes</h4>';
$admin .='<div class="border">';
$admin .= '<table  width="100%"><tr>';
$no =0;
$myDir = "content/themes/"; 
$dir = opendir($myDir);	
while($tmp = readdir($dir)){
	if($tmp != '..' && $tmp !='.' && $tmp !=''&& $tmp !='.htaccess'&& $tmp !='index.html'&& $tmp !='user'&& $tmp !='administrator'){
$urutan = $no + 1;
// read isi readme 
$fileComment = "content/themes/$tmp/$tmp.txt"; 
$f = fopen($fileComment, "r"); 
$isi = fread($f, filesize($fileComment)); 
fclose($f); 
$readmetmp = $isi;
//-----------------
$admin .= '<td valign="top"><img class="img-thumbnail pull-left" src="content/themes/'.$tmp.'/'.$tmp.'.jpg" width="200"><h4>'.$tmp.'</h4>'.$readmetmp.'';
