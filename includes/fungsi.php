...
function kotakjudul($title, $content) {
    global  $theme;
    $thefile = addslashes(file_get_contents("content/themes/".$theme."/boxmenu.html"));
...
function modul($posisi){
	...
	$numb = $koneksi_db->sql_query("SELECT * FROM mod_actions WHERE modul = '$pilih'");
...
$query		= $koneksi_db->sql_query ("SELECT * FROM users WHERE user='$user' AND password='$password' AND tipe='aktif'");
...
$query		= $koneksi_db->sql_query ("SELECT * FROM users WHERE user='$user' and email='$email' and hint='$hint' and hintjawab='$hintjawab' AND tipe='aktif'");
$total		= $koneksi_db->sql_numrows($query);
$data		= $koneksi_db->sql_fetchrow ($query);
...
//simpan file di folder yang anda tentukan sendiri. kalo saya sech folder "DATA"
	...
	$handle = fopen('./content/db-backup/'.$nama_file,'w+');
